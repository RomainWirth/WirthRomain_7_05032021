...
exports.signup = (req, res, next) => {
    bcrypt.hash(req.body.u_password, 10) // hash : salt = 10 tours d'algorythme de hashage suffisent pour la sécurité et éviter que ce soit trop long
    .then(hash => { // hash = méthode pour crypter le mot de passe
        const user = new User({
            email: req.body.u_email, // adresse email du corps de la requête
...
exports.login = (req, res, next) => {
    User.findOne({ email: req.body.u_email })
    .then(user => {
        if (!user) {
